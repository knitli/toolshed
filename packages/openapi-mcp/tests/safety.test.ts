...
describe("classifySafety", () => {
  test("GET and HEAD are reads", () => {
    expect(classifySafety("GET", "/widgets", "widgets.ListWidgets", "graph")).toBe("read");
    expect(classifySafety("HEAD", "/widgets", "widgets.Head", "graph")).toBe("read");
  });

  test("method matching is case-insensitive", () => {
    expect(classifySafety("get", "/widgets", "widgets.Do", "graph")).toBe("read");
  });

  test("POST, PATCH, PUT, DELETE are writes", () => {
    for (const m of ["POST", "PATCH", "PUT", "DELETE"]) {
      expect(classifySafety(m, "/widgets", "widgets.Do", "graph")).toBe("write");
    }
  });

  test("overrides reclassify semantically-read POSTs", () => {
    expect(classifySafety("POST", "/widgets/getByIds", "widgets.getByIds", "graph")).toBe("read");
    expect(
      classifySafety("POST", "/users/{id}/getMemberGroups", "users.user.getMemberGroups", "graph"),
    ).toBe("read");
    expect(
      classifySafety("POST", "/users/{id}/checkMemberObjects", "users.user.checkMemberObjects", "graph"),
    ).toBe("read");
  });

  test("overrides match a dotless operationId as its own tail", () => {
    expect(classifySafety("POST", "/x/getByIds", "getByIds", "graph")).toBe("read");
  });

  test("$batch is a write even though overrides might match", () => {
    expect(classifySafety("POST", "/$batch", "batch.Batch", "graph")).toBe("write");
    expect(isBatch("/$batch")).toBe(true);
    expect(classifySafety("POST", "/$batch", "widgets.getByIds", "graph")).toBe("write");
  });
  ...
  test("$batch is hard-pinned to write even on a GET", () => {
    ...
    expect(classifySafety("GET", "/$batch", "batch.Batch", "graph")).toBe("write");
  });
  ...
  test("an API with no override entry falls back to write, not to graph's list", () => {
    ...
    expect(classifySafety("POST", "/x/query", "x.query", "unregistered")).toBe("write");
  });

  test("overrides never promote a GET to a write", () => {
    expect(classifySafety("GET", "/widgets/getByIds", "widgets.getByIds", "graph")).toBe("read");
  });
});
