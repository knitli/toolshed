...
export interface PreparedCall {
  ...
  origin: string;
  relativeUrl: string;
  /** Opaque continuation proof; null on first-page reads and all actions. */
...
The local public constructor is `createLocalDispatchBoundary(authorizer, options)`, returning the paired broker, `AuthorizedTransport`, shared `paginationTokenCodec`, and local `close(): Promise<void>`. It accepts no replaceable permit validator. The configured authorizer is the host policy authority; a recomputed credential binding digest checks integrity, not provider provenance. Options include a detached trusted `LocalAuthProfile` and mandatory `allowsManifestOrigin(context)` callback. Its frozen context is exactly `{ catalogId, releaseId, manifestDigest, origin }`; only literal `true` permits access, and errors are redacted denials. The host must implement this callback from verified manifests and current catalog policy, including in Task 11; a self-hashed prepared call does not establish signed origin authority. The callback is captured once and checked before DNS at preflight and on every allowed redirect or continuation, intersecting the profile's exact origins.

The boundary recomputes the configured profile digest and actual injection-slot digest, validates the credential mechanism/name/placement, configured audience, and live scopes. Environment scopes match exactly; OAuth scopes may be narrowed from the configured request scopes. Secret-bearing snapshots stay only in private weakly registered plan state. Plans expire after at most 30 seconds; TTL and runtime-limit overrides only reduce defaults. Closing a boundary invalidates unused plans and its token codec, interrupts pending manifest/DNS checks and active HTTP, and awaits private client destruction. This local lifecycle method does not change the portable transport port or introduce global request context. An action cancelled after connection starts retains outcome-unknown semantics.
...
The Node adapter pins MIT-licensed Undici 8.10.2 and uses its imported Fetch with an isolated HTTP/1 client per attempt, `allowH2: false`, `pipelining: 0`, and forced dispatcher-level `idempotent: false`. It installs no global dispatcher, proxy fallback, retry interceptor, or redirect interceptor. Its private TLS connector preserves the original hostname/SNI and ordinary certificate verification, observes TCP establishment directly, and uses only the approved lookup answers. Fetch uses `redirect: "manual"`; reads may follow at most three same-origin HTTPS redirects after renewed manifest/profile/DNS checks. All action redirects and all cross-origin/downgrade redirects return `{ kind: "redirect-blocked", location: null }` without a second request, body, or credential injection. Authorization, proxy authorization, cookies, `Host`, connection/transfer/upgrade headers, forwarding headers, and provider identity headers are denied from model/runtime inputs. GET and HEAD request bodies are explicitly rejected before DNS because this Fetch adapter cannot transmit them; read dispatch also rejects non-read methods.

Content length is checked before reading and decompressed bytes are counted while streaming. HEAD has no response body, so its hypothetical GET Content-Length does not consume the byte budget. Unsupported encodings and chains that Undici would leave encoded are rejected. The request deadline, response limit, page limit, and cumulative pagination bytes use §6. Reads retry only 429/502/503/504 or positively established pre-connect failures, at most twice within the original deadline, honoring Retry-After only when it fits. Actions never retry. TCP-connected TLS failures, disconnects, timeouts, or cancellation after connection starts return `UPSTREAM_OUTCOME_UNKNOWN` with only the prepared-call digest; only positively established pre-connect failures can be reported as ordinary upstream failures.

Initial pagination recognizes standard HTTP `Link` with `rel=next` only. It does not guess provider JSON fields or interpret unsigned advisory metadata as extraction authority; body-only pagination needs later explicit trusted configuration. Continuation URLs must be canonical, credential-free, same-origin HTTPS and pass manifest/profile/DNS checks. Model-facing response headers exclude Link, Location, and cookies. The shared codec stores all `PaginationTokenState` fields in a bounded process registry (capacity 1024, TTL 120 seconds; lower-only overrides) indexed by random opaque IDs authenticated with a process HMAC. The token does not base64-encode the URL. Registry eviction or process restart invalidates outstanding tokens.

The unpublished prepared-call v2 representation now includes nullable `pageToken`, committed together with selected `relativeUrl` by `preparedCallDigest`; `inputDigest` remains tied only to normalized original arguments. The runtime's existing prepareRead/revalidate path decodes and defensively validates token shape, expiry, tuple, URL, reserved credential slots and page/byte limits against a freshly prepared base call. Revalidation decodes the retained proof again, so arbitrary alternate URLs or copied calls cannot replace token authority. Actions reject continuation tokens. Legacy v4 fixture bytes and artifact versions remain unchanged; publication before this shape amendment would instead require a prepared-call version migration.
